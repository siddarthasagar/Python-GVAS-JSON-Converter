    ...
    try:
        ...
        for i, node in enumerate(nodes):
            if "Struct" in node and "SavedFoodHeld_0" in node["Struct"]:
                food_values.append({"node_id": i, "food_value": node["Struct"]["SavedFoodHeld_0"]["Int"]})
        return food_values
    except Exception as e:
...
def extract_food_nodes_uesave_format(data):
    ...
    try:
        ...
        for item in data:
            if isinstance(item, dict) and item.get("name") == "ResourceBaseSaveStates" and "value" in item:
                nodes = item["value"]
                food_values = []
                for i, node in enumerate(nodes):
                    ...
                    if isinstance(node, dict):
                        ...
                        if food_val is not None:
                            food_values.append({"node_id": i, "food_value": food_val})
                return food_values
        return []
    except Exception as e:
...
def main():
    ...
    if command == "validate" and len(sys.argv) >= MIN_ARGS_VALIDATE:
        old_json = sys.argv[2]
        uesave_json = sys.argv[3]
        success = compare_workflows(old_json, uesave_json)
        sys.exit(0 if success else 1)

    elif command == "check" and len(sys.argv) >= MIN_ARGS_CHECK:
